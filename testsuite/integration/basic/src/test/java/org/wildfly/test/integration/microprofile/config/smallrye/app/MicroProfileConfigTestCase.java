...
public class MicroProfileConfigTestCase {
    ...
    // TODO also check that server log contains no error...?
    ...
    public void testGetWithConfigProperties() throws Exception {
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            // TODO - this is weird... server shows different path to what is retrieved here:
            // - server: /home/jstourac/workspace/wildfly/testsuite/integration/basic/target/wildfly
            // - this:   /home/jstourac/workspace/wildfly/testsuite/integration/basic/../../../build/target/wildfly-14.0.0.Beta1-SNAPSHOT
            assertTextContainsProperty(text, "jboss.home", System.getProperty("jboss.home"));
            assertTextContainsProperty(text, "JBOSS_HOME", System.getenv("JBOSS_HOME"));
        }
    }
    ...
    public void testGetBooleanProperties() throws Exception {
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            assertTextContainsProperty(text, "boolNoDefault", false);
            ...
            assertTextContainsProperty(text, "booleanNoDefault", false);
    ...
    public void testGetIntegerProperties() throws Exception {
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            assertTextContainsProperty(text, "intNoDefault", 0); // TODO - does it make sense to check default values?
            ...
            assertTextContainsProperty(text, "integerNoDefault", "null");
    ...
    public void testGetLongProperties() throws Exception {
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            assertTextContainsProperty(text, "longNoDefault", 0); // TODO - does it make sense to check default values?
            ...
            assertTextContainsProperty(text, "longClassNoDefault", "null");
    ...
    public void testGetFloatProperties() throws Exception {
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            assertTextContainsProperty(text, "floatNoDefault", 0.0f); // TODO - does it make sense to check default values?
            ...
            assertTextContainsProperty(text, "floatClassNoDefault", "null");
    ...
    public void testGetDoubleProperties() throws Exception {
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            assertTextContainsProperty(text, "doubleNoDefault", 0.0d); // TODO - does it make sense to check default values?
            ...
            assertTextContainsProperty(text, "doubleClassNoDefault", "null");
...
//            Assert.assertTrue(text.contains("myPetsOverridden as String set = [donkey,shrek]") || text.contains("myPetsOverridden as String set = [shrek,donkey]"));
    ...
    /**
     * Check String array, List and Set properties are correctly handled if they are not set and have no default values.
     *
     * @throws Exception
     */
    @Test
    public void testGetWithArraySetListNoDefaultProperties() throws Exception {
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            HttpResponse response = client.execute(new HttpGet(url + appContext + TestApplication.ARRAY_SET_LIST_NO_DEF_APP_PATH));
            Assert.assertEquals(200, response.getStatusLine().getStatusCode());
            String text = getContent(response);

            assertTextContainsProperty(text, "myPetsNonDef as String array", Arrays.toString(new String[] {}));
            assertTextContainsProperty(text, "myPetsNonDef as String list", new LinkedList<>());
            assertTextContainsProperty(text, "myPetsNonDef as String set", new HashSet<>());
        }
    }
    ...
    public void testPriorityOrderingProperties() throws Exception {
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            // TODO - this check fails but not sure why, yet - have to go now, will investigate tomorrow...
            ...
            // TODO we should add also test for properties with same priority but from different sources
        }
    }
}
